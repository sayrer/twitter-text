...
# Ruby-only benchmark for twitter-text
...
# Load Ruby implementation
lib_path = File.expand_path('../../rb/lib', File.dirname(__FILE__))
...
def benchmark_autolink(data)
  tests = data['tests']
  autolinker = Object.new.extend(Twitter::TwitterText::Autolink)
...
def benchmark_extract(data)
  ...
  extractor = Object.new.extend(Twitter::TwitterText::Extractor)
...
def benchmark_validate_tweet(data)
  ...
  validator = Object.new.extend(Twitter::TwitterText::Validation)

  # Warmup
  WARMUP_ITERATIONS.times do
    tweets.each { |test| validator.parse_tweet(test['text'])[:valid] }
  end
...
def benchmark_parse(data)
  ...
  # Warmup
  WARMUP_ITERATIONS.times do
    tests.each { |test| validator.parse_tweet(test['text']) }
  end
  ...
  print_results("Parse Tweet", ops_per_sec)
end
...
# Main
puts "Twitter Text Ruby-Only Benchmark"
puts "================================="
